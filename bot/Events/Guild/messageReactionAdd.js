...
module.exports = {
  ...
  async execute(bot, reaction, user){
    try {
      ...
      if(!member) return;
      if(member.roles.cache.has(match.roleId)) return; // already has
      await member.roles.add(match.roleId, 'Reaction role add').catch(()=>{});
    } catch(err){
      console.error('messageReactionAdd handler error:', err?.message||err);
    }
  }
};
