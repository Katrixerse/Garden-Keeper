...
module.exports = {
    ...
    run: async (bot, prefix, message, args) => {
        ...
        if (!invoker.permissions.has(PermissionsBitField.Flags.KickMembers))
            return message.reply("Missing permission: Kick Members.");
        if (!me.permissions.has(PermissionsBitField.Flags.KickMembers))
            return message.reply("I need Kick Members permission.");

        const targetMention = args[0];
        if (!targetMention) return message.reply(`Usage: ${prefix || "!"}kick @user [reason]`);
        ...
        if (invoker.id === target.id) return message.reply("You cannot kick yourself.");
        ...
        await target.kick(`${invoker.user.tag}: ${reason}`).catch(err => message.reply(`Kick failed: ${err?.message || err}`));
        ...
        try {
            ...
            const caseId = await logAction(message.guild.id, 'kick', { targetId: target.id, moderatorId: invoker.id, reason });
            if(a.enabled && a.channelId){
                const ch = message.guild.channels.cache.get(a.channelId);
                if(ch && ch.isTextBased()) ch.send(`🛠️ Kick • Case #${caseId || '?'} • **${target.user.tag}** (${target.id}) by **${invoker.user.tag}** • ${reason}`).catch(()=>{});
            }
        } catch {}
    }
};