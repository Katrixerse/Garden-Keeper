...
// Get 1-based rank position of a user within a guild based on (level DESC, xp DESC)
async function getUserRankPosition(guildId, userId){
  await initLevelsTable();
  const [rows] = await conn.promise().query('SELECT level, xp FROM user_levels WHERE guild_id=? AND user_id=?',[guildId,userId]);
  if(!rows.length) return null; // user not on board yet
  const { level, xp } = rows[0];
  const [higher] = await conn.promise().query('SELECT COUNT(*) AS c FROM user_levels WHERE guild_id=? AND (level > ? OR (level = ? AND xp > ?))',[guildId, level, level, xp]);
  return higher[0].c + 1;
}

// Developer badge support
// Configure developer IDs via env var DEVELOPER_IDS="id1,id2" or by editing this array fallback.
const DEV_SET = new Set(['130515926117253122', '307472480627326987']);
function isDeveloper(userId) { 
    return DEV_SET.has(userId); 
}

module.exports = { addXP, getLevel, getLeaderboard, getUserRankPosition, calcNeededXP, getBadges, getTitleForLevel, getRankCardBackground, setRankCardBackground, PRESET_BACKGROUNDS, isDeveloper, calcCumulativeXP };
